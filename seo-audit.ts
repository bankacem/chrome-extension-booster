...
function run(): void {
  ...
  if (existingImageDirs.length === 0)
    console.log("  (none found — check configuration)");
  ...
  list("Broken internal links", brokenLinks.map((r) => `${r.relPath} -> ${r.brokenInternal.join(", ")}`));
  list("Malformed external links", malformedExt.map((r) => `${r.relPath} -> ${r.malformedExternal.join(", ")}`));
  list("Broken images", brokenImages.map((r) => `${r.relPath} -> ${r.missingImages.join(", ")}`));

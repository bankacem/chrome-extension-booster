...
// First publish date (today). One article per subsequent day.
const START_DATE = '2026-06-07';

/**
 * Returns an ISO date string (YYYY-MM-DD) offset by `days` from START_DATE.
 */
...
/**
 * Lists .md files located DIRECTLY inside public/content/ (non-recursive),
 * excluding the partitioned `articles/` directory and any other subdirectories.
 */
...
function schedule() {
  ...
  files.forEach((filePath, index) => {
    try {
      ...
      // Determine slug: frontmatter slug, else filename.
      ...
      if (APPLY) {
        ...
        // Remove the original loose file if it was relocated.

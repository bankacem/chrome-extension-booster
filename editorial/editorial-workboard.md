...
### tp004 — `chrome-omnibox-guide`

- **Keyword and intent:** `omniboxes`; a split-path guide for Chrome built-in address-bar/site-search shortcuts and extension keyword actions. The article deliberately avoids becoming a generic browser-tips or benefits/risks page.
- **Agent run and remediation:** GitHub Actions run [32598239714](https://github.com/bankacem/chrome-extension-booster/actions/runs/32598239714) ran `seo_agent_pro` with the explicit keyword. Agent PR #331 was closed as superseded because it used an unreserved slug, a generic title/intent, and modified shared index/Sitemap files. Its final status was `needs_human_review`; no automatic agent content was merged.
- **Accepted content:** human-reviewed PR #332 on branch `refine/chrome-omnibox-guide`, merge commit `90aaa808`. The article path is `public/content/articles/c/h/r/chrome-omnibox-guide.md`; the slug-derived `c/h/r` partition was checked. The article includes the decision table, exact Chrome Settings path, extension keyword flow, compact Manifest V3/API example, troubleshooting, privacy and managed-device cautions, visible FAQ/HowTo content, official references, and four verified internal links.
- **Integration:** PR #333 updated only `public/content/articles-index.json`, `public/content/sitemap-dates.json`, and `public/sitemap.xml` after the content merge. Main commit is `dab621c8`. The exact slug occurs once in the index, the canonical URL occurs once in the Sitemap, and the rejected agent slug is absent from both.
- **Validation:** local and GitHub quality gates passed: sync, build, typecheck, performance budgets, SEO smoke tests, internal-link smoke tests, and `git diff --check`. The final local SEO smoke test reported 786 Sitemap URLs, 754 English articles, 9 extensions, and 20 localized articles; the link test scanned 8,029 links with 0 redirect links and 6 documented legacy exceptions. Prerendered HTML was 20,304 bytes with one `Article`, one `BreadcrumbList`, one `FAQPage`, and one `HowTo` schema.
- **Live validation:** Production Vercel deployment `dpl_3XC7rNoQgRwuUphKiytQyeqhCfcx` for verified main commit `dab621c8` reached `READY`. The live URL `https://extensionto.com/blog/chrome-omnibox-guide` returned HTTP 200 with the expected H1, title, meta description, self-canonical, Article/BreadcrumbList/FAQPage/HowTo schemas, and internal links. An initial HTTP 404 occurred only while the deployment was queued and was not a source or route failure.
- **Decision:** tp004 is recorded as `merged` and technically live. This confirms publication, index/Sitemap inclusion, and live HTML correctness only; it is not evidence that Google has indexed or ranked the page. No Vercel plan, billing, or concurrent-build setting was changed.
